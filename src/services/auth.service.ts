...
class AuthService {
  ...
  public async signup(clientData: CreateClientDto): Promise<Client> {
    ...
    if (findClient)
      throw new HttpException(
        409,
        `You're email ${clientData.email} already exists`
      );
  ...
  public async login(
  ...
  ): Promise<{ cookie: string; findClient: Client }> {
    if (isEmpty(clientData))
      throw new HttpException(400, "You're not clientData");
    ...
    if (!findClient)
      throw new HttpException(
        409,
        `You're email ${clientData.email} not found`
      );
  ...
  public async logout(clientData: Client): Promise<Client> {
    ...
    const findClient: Client = await this.clients.findOne({
      where: { email: clientData.email, password: clientData.password },
    });
